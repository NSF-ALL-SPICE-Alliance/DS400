...
ggplot(df, aes(x = DRD370B, y = LBXBGM, fill = DRD370B)) +
  geom_violin(alpha = 0.7, trim = FALSE) +  # Keep full distribution
  geom_boxplot(width = 0.1, color = "black", alpha = 0.7, outlier.shape = NA) +  # Overlay boxplot
  stat_summary(fun = median, geom = "point", size = 3, color = "black") +  # Show median
  scale_fill_manual(values = c("red3", "steelblue")) +  # Custom colors
  labs(
    x = "Ahi Consumption",
    y = "Blood Mercury Levels (µg/L)",
    title = "Distribution of Blood Mercury Levels by Ahi Consumption"
  ) +
  theme_minimal(base_size = 14) +  # Use clean theme
