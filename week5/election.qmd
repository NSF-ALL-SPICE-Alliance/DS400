...
1.  Describe and compare both the prior and likelihood function in words.

    Prior function shows what the previous variability was, this being lower with greater density. From this it looks like the prior was around 95-99%.

    Likelihood function is the new information that is inputted with the new data. This shows a higher variability with lower density. Looks like the mean is around 22-25%

2.  Describe the posterior model in words. Does it more closely agree with the data (as reflected in the likelihood function) or the prior?

    It more agrees with the prior.

3.  Provide the specific plot_beta_binomial() code you would use to produce a similar plot.

    ```{r}
    plot_beta_binomial(55, 2, y = 6, n = 25)
    ```
