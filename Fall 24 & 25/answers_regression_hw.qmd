...
ggplot(penguins_complete, aes(x = body_mass_g, fill = species)) +
  geom_boxplot()
...
penguin_model_1 <- stan_glm(body_mass_g ~ flipper_length_mm, data = penguins_complete, 
                            family = gaussian,
...
penguin_model_2 <- stan_glm(body_mass_g ~ species, data = penguins_complete, 
                            family = gaussian,
...
penguin_model_3 <- stan_glm(body_mass_g ~ flipper_length_mm + species, data = penguins_complete, 
                            family = gaussian,
                            prior_intercept = normal(0, 2500),
...
penguin_model_4 <- stan_glm(body_mass_g ~ flipper_length_mm + bill_length_mm + bill_depth_mm, data = penguins_complete, 
                            family = gaussian,
                            prior_intercept = normal(0, 2500),
                            prior = normal(0, 1000, autoscale = TRUE) , 
                            prior_aux = exponential(1/500),
...
Create a comparison table in your write-up:

| Model | Predictors        | MAE (grams) | Interpretation |
|-------|-------------------|-------------|----------------|
| 1     | flipper only      | ???         | 269.2525       |
| 2     | species only      | ???         | 338.2141       |
| 3     | flipper + species | ???         | 264.8568       |
| 4     | all measurements  | ???         | 269.9035       |
...
**Hints & Resources:** - Review the weather Perth rain examples from class - For proportions, structure like: `r   fake_news %>%      count(title_has_excl, type) %>%     group_by(title_has_excl) %>%     mutate(prop = n / sum(n))`
:::
...
fake_news_model<- stan_glm(type ~ title_has_excl + title_words + negative, data = fake_news, 
                            family = binomial,
...
After printing your model: - **Intercept**: Log-odds of fake news at baseline (all predictors = 0) - **Coefficients**: Change in log-odds per unit increase in predictor - Positive coefficient = increases odds of fake news - Negative coefficient = decreases odds of fake news
:::
...
**What are Odds Ratios?** - Odds Ratio (OR) = exp(coefficient) - OR \> 1 means predictor **decreases** odds of fake news - OR \< 1 means predictor **increases** odds of fake news - OR = 1 means no effect

**Calculate 80% Credible Intervals:**

Use: `exp(posterior_interval(fake_news_model, prob = 0.80))`

**Interpret the results:** - For `title_has_exclTRUE`: How much do exclamation points multiply the odds of fake news? - For `title_words`: For each additional word in the title, how do the odds change? - For `negative`: For each 0.1 increase in negative sentiment, how do the odds change?

**Hints & Resources:** - Review the rain model interpretation from class - To convert OR to percent change: `(OR - 1) * 100` - Example: OR = 1.25 means +25% increase in odds
:::
...
For each predictor, write 1-2 sentences interpreting the odds ratio:

1.  **title_has_excl**: "Articles with exclamation points have odds of being fake that are \[X times / X% higher/lower\] compared to articles without exclamation points (80% credible interval: \[low, high\])."

2.  **title_words**: "For each additional word in the title, the odds of fake news \[increase/decrease\] by approximately \[X%\]."

3.  **negative**: "For each 0.1 unit increase in negative sentiment, the odds of fake news \[increase/decrease\] by approximately \[X%\]."
:::
...
2.  What is the sensitivity? Are we good at catching fake news?
3.  What is the specificity? Are we good at identifying real news?
4.  Which type of error is more problematic: false positives or false negatives?
:::
...
**The Trade-off:** - **Lower cutoff** (e.g., 0.3): Easier to classify as Fake - ✅ Catches more fake news (higher sensitivity) - ❌ More false alarms (lower specificity)

-   **Higher cutoff** (e.g., 0.7): Harder to classify as Fake
    -   ❌ Misses more fake news (lower sensitivity)
    -   ✅ Fewer false alarms (higher specificity)

**Try these cutoffs and compare:** 1. `cutoff = 0.3` (aggressive fake news detection) 2. `cutoff = 0.7` (conservative fake news detection)
...
| Cutoff | Accuracy | Sensitivity | Specificity | Best For... |
|--------|----------|-------------|-------------|-------------|
| 0.3    | ???      | ???         | ???         | ???         |
| 0.5    | ???      | ???         | ???         | ???         |
| 0.7    | ???      | ???         | ???         | ???         |

**Discussion:** Which cutoff would you choose if: - You run a fact-checking website (want to catch all fake news)? - You're a news aggregator (want to avoid censoring real news)?
:::
...
fake_news %>%
  ...
  ggplot(aes(x = title_words, y = type)) +
    ...
    labs(y = "probability of fake news")
```

**Option 2: Faceted Plot**

Show how predictions differ by exclamation point presence: - Create the above plot - Add `facet_wrap(~title_has_excl)`
...
What does this plot tell you about: 1. The relationship between title length and fake news probability? 2. How does this relationship differ for articles with/without exclamation points? 3. Where is the model most/least certain?
:::
...
1.  **Feature Importance**: Which predictor(s) were most strongly associated with fake news? How can you tell from the odds ratios?

2.  **Classification Trade-offs**: In the context of fake news detection, is it worse to:

    -   Flag real news as fake (false positive)?
    -   Miss fake news (false negative)?
    -   How should this influence your cutoff choice?

3.  **Model Limitations**: What information is this model missing? What other predictors might be useful?

4.  **Causality Warning**: Can you say that exclamation points **cause** fake news? Why or why not?

5.  **Real-world Application**: How would you deploy this model? What additional validation would you need?
:::
