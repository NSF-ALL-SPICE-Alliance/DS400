...
```{r}
write_csv(df, here("data/nhanes_data/all_data_joined.csv"))
```

```{r}
df_test <- read_csv(here("data/nhanes_data/all_data_joined.csv"))
```
...
```{r}
data_joined <- full_join(demographics, tiid, by = "SEQN")
```

```{r}
data_joined <- data_joined %>% 
  mutate(age_threshold = ifelse(RIDAGEYR >= 40, "over_40", "under_40"))
```
...
```{r}
ggplot(data_joined, aes(x = RIDAGEYR)) +
  geom_histogram()
```

```{r}
data_joined %>% 
  tabyl(age_threshold, DIQ010) %>% 
