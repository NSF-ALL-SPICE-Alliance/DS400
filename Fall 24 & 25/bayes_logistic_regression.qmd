...
37% of bookings are canceled in the dataset

```{r}
canceled_bookings <- 0.37
```

1.  Exploratory Data Analysis: Construct and discuss plots of `is_canceled` vs each of the four potential predictors above
...
```{r}
ggplot(hotel_data, aes(x = lead_time, color = is_canceled, fill = is_canceled)) +
  geom_histogram()
```

```{r}
ggplot(hotel_data, aes(x = average_daily_rate, color = is_canceled, fill = is_canceled)) +
  geom_histogram()
```

1.  Utilize `stan_glm()` to create a bayesian logistic regression model with is_canceled as your target/response variable
...
```{r}
cancelation_model <- stan_glm(
  is_canceled ~ previous_cancellations + lead_time + is_repeated_guest, average_daily_rate, 
...
1.  Use `exp(posterior_interval()` to interpret key predictors

```{r}
exp(posterior_interval(cancelation_model, prob = 0.8)) 
```

1.  Use `classification_summary` to interpret model accuracy

    1.  Use different values for the cutoff argument and see how sensitivity and specificity change

```{r}
classification_summary(model = cancelation_model, data = hotel_data, cutoff = 0.4)
```
