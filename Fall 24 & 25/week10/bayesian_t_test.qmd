...
```{r}
ggplot(data = bikes, aes(x = holiday, y = rides, fill = holiday)) +
  geom_boxplot() +
  geom_jitter(size = 0.1) +
  theme_bw()
```
...
bikes_cat_1_2 <- bikes %>%
  ...
  mutate(weather_cat = factor(weather_cat, levels = c("categ1", "categ2")))

```

```{r}
ggplot(data = bikes_cat_1_2, aes(x = weather_cat, y = rides, fill = weather_cat)) +
  geom_boxplot() +
  geom_jitter(size = 0.1) +
  theme_bw()
```
...
```{r}
library(brms)
model <- brm(LBXBGM ~ DRD370B, data = df, family = gaussian())
summary(model)
```
